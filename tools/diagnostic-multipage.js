...
(async () => {
  ...
  // Let's create a full 2-page resume
  const experiences = Array.from({ length: 6 }).map((_, i) => `
    <div class="rp-item">
      <div class="rp-item-head">
        <strong>Senior Clinical Data Analyst ${i + 1}</strong>
        <span>Jan 202${i} - Dec 202${i + 1}</span>
      </div>
      <div class="rp-item-sub">
        <span>Healthcare System ${i + 1}</span>
        <span>Boston, MA</span>
      </div>
      <div class="rp-item-desc">
        <ul class="rp-desc-list">
          <li>Engineered automated SQL pipelines for clinical quality measure reporting across multiple hospital units.</li>
          <li>Reduced EHR reporting discrepancies by 34% using predictive validation models and machine learning pipelines.</li>
        </ul>
      </div>
    </div>
  `).join('');
  ...
  <head>
    <meta charset="utf-8">
    <title>Multi-Page Resume Test</title>
    <style>
      ${css}
    </style>
    <style>
      ...
      <div class="rp-ats-container rp-ats-classic">
        ...
        <main class="rp-ats-body">
          <section class="rp-section">
            ...
            <p>Healthcare Data Analyst with 10+ years of experience optimizing EHR workflows and clinical reporting.</p>
          </section>
          <section class="rp-section">
  ...
  const outPdf = path.resolve(__dirname, 'multipage-test.pdf');
  await page.pdf({
    path: outPdf,
    format: 'letter',
  ...
  console.log('Multi-page Resume PDF Page count:', doc.getPageCount());
